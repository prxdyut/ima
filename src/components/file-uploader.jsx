...
export default function UploadComponent({
  ...
  useEffect(() => {
    onChange(images);
    buttonOnlyImageControls(<FilesViewerNative contents={images || []} update={setImages} />)
  }, [images]);
  ...
  async function uploadDocuments(url, { arg }) {
    const fileToBlobs = (await Promise.all([...arg.files].map(async file => ({ file: file.type == 'application/pdf' ? file : await fileToBlob(file), fileName: file.name, type: file.type }))))
    const compressedBlobs = (await Promise.all(fileToBlobs.map(async ({ file, fileName, type }) =>
      type == 'application/pdf' ? ({ file, fileName, type }) : ({ file: await handleBlob(file), fileName })
    )))
    const compressedImages = (compressedBlobs.map(({ file, fileName }) => new File([file], fileName)))
    ...
    const response = await fetch(url, { method: "POST", body }).then((res) =>
      res.json()
    ).catch(e => { setUploading(false); alert(JSON.stringify(e)) })
  ...
  return (
    <Box>
      <ImagePicker />
      {!buttonOnly && <FilesViewerNative contents={images || []} update={setImages} />}
...
const FilesViewerNative = ({ contents = [], update }) => {
  ...
  const deleteItem = (key, idx) => {
    ...
    update(contents.filter((file, i) => i != idx));
  };
...
// const PDFViewer = ({ contents = [], update }) => {
//   const searchParam = useSearchParams();
//   const pathname = usePathname();
//   const router = useRouter();
//   const deleteItem = (key, idx) => {
